...
from src.core.evidence import single_segment_audio_likelihood, get_compressed_audio_from_path
...
class PathSampler:
    ...
    def sample_paths(self, task: SimulationConfig) -> Dict:
        """Main entry point for path sampling with new config system"""
        
        if task.evidence.type == 'visual':
            return self._sample_visual_paths(task)
        elif task.evidence.type == 'audio':
            return self._sample_audio_paths(task)
        else:
            raise ValueError(f"Unsupported evidence type: {task.evidence.type}")
    
    def _sample_visual_paths(self, task: SimulationConfig) -> Dict:
        ...
        for _ in range(task.num_sample_paths):
            ...
            if task.agent_type == 'sophisticated' and optimal_plant_spots[idx2] is not None:
                ...
                if task.sampling.noisy_planting_sigma > 0:
                    chosen_plant_spot = self._get_noisy_plant_spot(
                        task.world, optimal_spot, task.sampling.noisy_planting_sigma
                    )
                else:
    ...
    def _sample_audio_paths(self, task: SimulationConfig) -> Dict:
        ...
        if task.agent_type == 'sophisticated':
            ...
            for idx, p2_seq in enumerate(sequences_p2):
                optimal_spot, best_slider = self._calculate_optimal_plant_spot_and_slider(
                    task.world, task.agent_id, p2_seq, fridge_access_point, task
                )
                optimal_plant_spots[idx] = optimal_spot
                
                path_framing_metric_scaled = best_slider / 100.0
                utility_factor = task.sampling.cost_weight * (1 - rescaled_lengths[idx])
                
                if task.agent_id == 'A':
                    utility = utility_factor + (1 - task.sampling.cost_weight) * path_framing_metric_scaled
                else:
                    utility = utility_factor - (1 - task.sampling.cost_weight) * path_framing_metric_scaled
            ...
            for l_rescaled in rescaled_lengths:
                utilities.append(task.sampling.cost_weight * (1 - l_rescaled))
    ...
    def _calculate_single_audio_utility(
        ...
        if task.agent_type in ['naive', 'uniform']:
            return task.sampling.cost_weight * (1 - rescaled_cost)
            ...
            if cache_key in self.audio_framing_cache:
                ...
                L_A_total, L_B_total = self._compute_audio_framing_likelihoods(
                    eval_steps_to, eval_steps_from, task
                )
            ...
            cost_utility_comp = task.sampling.cost_weight * (1 - rescaled_cost)
            framing_utility_comp = (1 - task.sampling.cost_weight) * path_framing_metric_scaled
    ...
    def _utilities_to_probabilities(
        ...
        temp = task.sampling.naive_temp if task.agent_type == 'naive' else task.sampling.sophisticated_temp
