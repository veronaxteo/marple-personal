...
# def generate_ground_truth_audio_sequences(world: 'World', cfg: Simulationcfg) -> list:
def generate_ground_truth_audio_sequences(world, cfg: SimulationConfig) -> list:
    """
    ...
    base_steps_to = int(min_steps_to_fridge) if min_steps_to_fridge != float('inf') else cfg.cfg.sampling.max_steps + 1
    base_steps_from = int(min_steps_from_fridge) if min_steps_from_fridge != float('inf') else cfg.cfg.sampling.max_steps + 1
    
    fridge_events = ['fridge_opened', 'snack_picked_up', 'fridge_closed']

    # Generate variations around base paths
    for to_increment in range(0, cfg.cfg.sampling.max_steps + 1, cfg.audio_gt_step_size):
        steps_to = base_steps_to + to_increment
        if steps_to > cfg.cfg.sampling.max_steps:
            continue

        for from_increment in range(0, cfg.cfg.sampling.max_steps + 1, cfg.audio_gt_step_size):
            steps_from = base_steps_from + from_increment
            if steps_from > cfg.cfg.sampling.max_steps:
                continue
    ...
    if not ground_truths and base_steps_to <= cfg.cfg.sampling.max_steps and base_steps_from <= cfg.cfg.sampling.max_steps:
