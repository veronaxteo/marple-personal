...
def plot_detective_predictions_heatmap(trial_name, param_log_dir, detective_agent_type, evidence_type='visual'):
    ...
    For visual evidence type, uses world coordinates to show full apartment context for debugging smoothing.
    For audio evidence type, this function may not be applicable as audio evidence type doesn't have spatial crumb locations.
    """
    logger = logging.getLogger(__name__)
    logging.basicConfig(level=logging.INFO)
    ...
    # Detective predictions are typically for visual evidence type with crumb locations
    # For audio evidence type, this heatmap doesn't make sense as predictions are about audio sequences, not spatial locations
    if evidence_type == 'audio':
        logger.info(f"Skipping detective predictions heatmap for audio evidence type (no spatial crumb locations)")
        return
    ...
    for pred_entry in predictions_data:
        ...
        # Try new data structure first (from VisualProcessor)
    ...
    try:
        ...
        for pred_entry in naive_predictions:
            ...
            if "crumb_coord" in pred_entry:
                ...
                world_coord_tuple = tuple(pred_entry["crumb_location_world_coords"])
                likelihood_A = pred_entry.get("cfg.evidence.type_likelihood_A")
                likelihood_B = pred_entry.get("cfg.evidence.type_likelihood_B")
...
def plot_crumb_planting_comparison(trial_name, param_log_dir, detective_agent_type, evidence_type='visual'):
    ...
    Only works for visual evidence type with spatial crumb locations.
    """
    logger = logging.getLogger(__name__)
    ...
    for agent_id in ['A', 'B']:
        ...
        for pred_entry in predictions_data:
            ...
            if "crumb_coord" in pred_entry:
                ...
                world_coord_tuple = tuple(pred_entry["crumb_location_world_coords"])
                likelihood = pred_entry.get(f"cfg.evidence.type_likelihood_{agent_id}")
    ...
    for agent_type in agent_types_for_path_plots:
        ...
        for csv_file_path in csv_files:
            try:
                ...
                # Determine evidence type type by trying to read metadata.json
                ...
                if not os.path.exists(metadata_path):
                    ...
                    if os.path.exists(metadata_path_alt):
                        ...
                        if os.path.exists(metadata_path_alt_2):
                            ...
                            logger.warning(f"metadata.json not found in {param_log_dir}, {os.path.dirname(param_log_dir)}, or {os.path.dirname(os.path.dirname(param_log_dir))}. Defaulting to visual evidence type type.")

                if os.path.exists(metadata_path):
                    try:
                        with open(metadata_path, 'r') as f:
                            metadata = json.load(f)
                        evidence_type = metadata.get('evidence_type', 'visual').lower()
                        logger.info(f"Determined evidence type type: {evidence_type} from {metadata_path}")
                    except Exception as e:
                ...
                if evidence_type == 'audio':
                    ...
                    # Plot both path segments for visual evidence type, just like audio
                ...
                if trial_context_key not in processed_trials_for_detective_plots:
                    for detective_model_type in detective_model_types_for_plots:
                        ...
                        if os.path.exists(json_pred_path):
                            logger.info(f"Plotting detective predictions for model '{detective_model_type}', trial '{trial_name}', evidence type '{evidence_type}'.")
                    ...
                    # Generate retrospective smoothing comparison plots for visual evidence type
                    if evidence_type == 'visual':
