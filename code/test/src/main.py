...
from src.core.simulation import RSMSimulator, EmpiricalSimulator, UniformSimulator
# from src.configs.cfg import Simulationcfg
from src.configs import SimConfig
from src.evaluation.plot import main as plot
...
def parse_args():
    ...
    simulate.add_argument('--cfg.evidence.type', type=str, default='visual', choices=['visual', 'audio'], help='which cfg.evidence.type to use (default: `visual`)')
    ...
    empirical.add_argument('--cfg.evidence.type', type=str, default='visual', choices=['visual', 'audio'], help='which cfg.evidence.type to use (default: `visual`)')
    ...
    uniform.add_argument('--cfg.evidence.type', type=str, default='visual', choices=['visual', 'audio'], help='which cfg.evidence.type to use (default: `visual`)')
...
def logger(args):
    ...
    if args.command == 'rsm':
        param_log_dir = create_param_dir(log_dir_base, trial_id_for_log, args.w, args.cfg.sampling.naive_temp, args.cfg.sampling.sophisticated_temp, args.cfg.sampling.max_steps,model_type="rsm")
        ...
        param_log_dir = create_param_dir(
            ...
            cfg.sampling.max_steps=args.cfg.sampling.max_steps,
            model_type="uniform"
        )
...
if __name__ == '__main__':
    ...
    # cfg = Simulationcfg(**experiment_cfg)
    cfg = SimConfig(**experiment_cfg)
    main_logger.info(f"Processed Simulation parameters: {cfg}")

    metadata_filepath = os.path.join(param_log_dir, 'metadata.json')
    cfg_dict_for_metadata = asdict(cfg)
    ...
    cfg_dict_for_metadata.pop('naive_A_audio_sequences', None)
    cfg_dict_for_metadata.pop('naive_B_audio_sequences', None)
    cfg_dict_for_metadata.pop('paths', None)

    with open(metadata_filepath, 'w') as f_meta:
        json.dump(cfg_dict_for_metadata, f_meta, indent=4)

    if args.command == 'rsm':
        simulator = RSMSimulator(args, log_dir_base, param_log_dir, cfg, trials_to_run)
    elif args.command == 'empirical':
        simulator = EmpiricalSimulator(args, log_dir_base, param_log_dir, cfg, trials_to_run)
    elif args.command == 'uniform':
        simulator = UniformSimulator(args, log_dir_base, param_log_dir, cfg, trials_to_run)
    else:
